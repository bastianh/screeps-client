import {
  ...
  NullStorage,
  SubscriptionGroup,
} from "./screeps-connectivity/dist/index.js";
...
async function main() {
  ...
  group.add(
    client.stores.room.on("room:update", ({ room, gameTime, objects }) => {
      updateCount++;
      const ids = Object.keys(objects);
      console.log(
        `✓ room:update  room=${room} tick=${gameTime} objects=${ids.length}`,
      );
      if (updateCount >= 3) {
        group.dispose();
        client.disconnect();
  ...
  setTimeout(() => {
    if (updateCount === 0) {
      console.error(
        `✗ No room:update received after 15s (subscribed to channel: ${shard ? `room:${shard}/${roomName}` : `room:${roomName}`})`,
      );
    }
    group.dispose();
    client.disconnect();
